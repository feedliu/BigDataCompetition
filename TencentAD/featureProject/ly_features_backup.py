...
def bayes_smoothing(x, y, alpha=alpha, beta=beta):
    '''
    x : 分子
    y : 分母
    '''
...
def get_app_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['app_install_ratio'] = bayes_smoothing(features['app_yes_num'], features['app_click_num'],
                                                        alpha=13573.0, beta=508464.0)
...
def get_position_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['position_install_ratio'] = bayes_smoothing(features['position_yes_num'], features['position_click_num'],
                                                                alpha=19315.0, beta=723563.0)
...
def get_camgaign_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['camgaignID_install_ratio'] = bayes_smoothing(features['yes_num'], features['camgaignID_click_num'],
                                                                alpha=1093.0, beta=42622.0)
...
def get_ad_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['ad_install_ratio'] = bayes_smoothing(features['yes_num'], features['adID_click_num'],
                                                        alpha=809.0, beta=31565.0)
...
def get_position_app_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['positionID_appID_install_ratio'] = bayes_smoothing(features['positionID_app_yes_num'], features['positionID_appID_click_num'],
                                                                    alpha=1497.0, beta=57195.0)
...
def get_position_creative_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['positionID_creativeID_install_ratio'] = bayes_smoothing(features['positionID_creativeID_yes_num'], features['positionID_creativeID_click_num'],
                                                        alpha=297.0, beta=11198.0)
...
def get_age_app_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features = features[['age', 'appID', 'age_appID_install_ratio', 'age_appID_click_num']]
        features.to_csv(dump_path,index=False, index_label=False)
    return features
...
def get_age_position_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features = features[['age', 'positionID', 'age_position_install_ratio', 'age_position_click_num']]
        features.to_csv(dump_path,index=False, index_label=False)
    return features
...
def get_user_position_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['user_position_install_ratio'] = bayes_smoothing(features['user_position_yes_num'], features['user_position_click_num'])
...
def get_connectionType_position_install_ratio_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['connectionType_position_install_ratio'] = bayes_smoothing(features['connectionType_position_yes_num'], features['connectionType_position_click_num'],
                                                            alpha=25546.0, beta=920602.0)
...
def get_user_click_install_ration_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features.fillna(0, inplace=True)
        features['user_install_ratio'] = bayes_smoothing(features['user_yes_num'], features['user_click_num'],
                                                        alpha=0.085, beta=4.0)
...
def get_user_has_installed_distance_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path): features = pd.read_csv(dump_path)
        ...
        features['installTime'] = time2minutes(end_date) - features['installTime'].map(time2minutes)
        features.to_csv(dump_path,index=False, index_label=False)
    return features
...
def make_train_set(start_date, end_date, test=False, sub=False):
    ...
    if os.path.exists(dump_path): actions = pd.read_csv(dump_path)
        ...
        actions = pd.merge(actions, get_creative_install_ratio_feat(history_start_date, history_end_date), on=['creativeID'], how='left')
        actions = pd.merge(actions, get_app_install_number_feat(history_start_date, history_end_date), on=['appID'], how='left')
        actions = pd.merge(actions, get_user_click_install_ration_feat(history_start_date, history_end_date), on=['userID'], how='left', suffixes=('_app', '_user'))
        actions = pd.merge(actions, get_user_install_app_number_feat(history_start_date, history_end_date), on=['userID'], how='left')
        ...
        actions = pd.merge(actions, get_age_position_install_ratio_feat(history_start_date, history_end_date),
                                on=['age', 'positionID'], how='left')
        ...
        # actions = pd.merge(actions, get_creative_active_time_feat(history_start_date, history_end_date),
        #                     on=['creativeID', 'clickHour'], how='left')
        
        # actions = pd.merge(actions, get_positionType_position_install_ratio_feat(history_start_date, history_end_date),
        #                     on=['positionID','positionType'], how='left')
        # actions = pd.merge(actions, get_app_active_hours_feat(history_start_date, history_end_date),
        #                         on=['appID'], how='left')
        # actions = pd.merge(actions, get_app_second_cate_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['app_second_cate'], how='left')
        # actions = pd.merge(actions, get_app_first_second_cate_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['app_first_cate', 'app_second_cate'], how='left')
        # actions = pd.merge(actions, get_app_first_cate_creative_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['app_first_cate', 'creativeID'], how='left')
        # actions = pd.merge(actions, get_app_first_cate_position_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['app_first_cate'], how='left')
        # actions = pd.merge(actions, get_gender_haveBaby_creative_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['gender', 'haveBaby', 'creativeID'], how='left')
        # actions = pd.merge(actions, get_age_gender_creativeID_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['age', 'gender', 'creativeID'], how='left')
        # actions = pd.merge(actions, get_connectType_creativeID_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['connectionType', 'creativeID'], how='left')
        # actions = pd.merge(actions, get_siteset_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['sitesetID'], how='left')
        # actions = pd.merge(actions, get_user_position_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['userID', 'positionID'], how='left')

        # actions = pd.merge(actions, get_education_position_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['education', 'positionID'], how='left')
        ...
        # if sub:
        #     actions = pd.merge(actions, get_user_has_installed_app_feat(history_start_date, start_date), on=['userID', 'appID'], how='left')
        # else:

        # actions = pd.merge(actions, get_user_click_num_feat(start_date, end_date), on=['userID'], how='left')
        actions = pd.merge(actions, get_the_num_of_user_has_clicked_feat(start_date, end_date,sub),
                            on=['userID', 'appID'], how='left')

        # for i in (1,2, 3, 5, 7): #days
        #     start_days = start_date - i * 10000
        #     actions = pd.merge(actions, get_the_num_of_user_has_clicked_feat(start_days, start_date), 
        #                         on=['userID', 'appID'], how='left', suffixes=('','_%s' % i))
            # actions = pd.merge(actions, get_creative_install_num(start_days, start_date), 
            #                     on=['creativeID'], how='left', suffixes=('','_%s' % i))


        actions.fillna(0, inplace=True)
        del actions['userID'] 
        # del actions['sitesetID'] 
        # del actions['app_click_num']
        # del actions['position_click_num']
        ...
        # del actions['app_first_cate']
        # del actions['telecomsOperator']

        labels = actions['label'].copy()
        del actions['label']

        # data cleaning
        # if (not test) and (not sub): #
        #     actions.replace(-1, np.nan, inplace=True)
        #     actions = actions.dropna(thresh=len(actions.columns) - 3)
        #     labels = labels.iloc[list(actions.index)]
        #     actions.fillna(-1,inplace=True)

    return actions, labels

def make_train_set_with_basic_feat(start_date, end_date, test=False, sub=False):
    dump_path = './cache/train_set.csv'
    if os.path.exists(dump_path): actions = pd.read_csv(dump_path)
    else:
        actions_path = './cache/actions_%s-%s.csv' % (start_date, end_date)
        ...
        # actions = pd.merge(actions, get_position_advertiseID_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['positionID', 'advertiserID'], how='left')
        # actions = pd.merge(actions, get_advertise_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['advertiserID'], how='left')
        # actions = pd.merge(actions, get_position_campaignID_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['positionID', 'camgaignID'], how='left')
        # actions = pd.merge(actions, get_position_ad_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['positionID', 'adID'], how='left')
        # actions = pd.merge(actions, get_siteset_position_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['sitesetID', 'positionID'], how='left')
        # actions = pd.merge(actions, get_siteset_positionType_install_ratio_feat(history_start_date, history_end_date),
        #                         on=['sitesetID', 'positionType'], how='left')
        ...
        actions.fillna(actions.mean(), inplace=True)
        # actions.fillna(0, inplace=True)
        del actions['userID']
...
if __name__ == '__main__':
    get_position_campaignID_install_ratio_feat(0, 24000000)
